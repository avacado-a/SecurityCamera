...
def handle_frame(message):
    ...
    # Add frame to buffer
    cameras[camera_name]['frame_buffer'].append(frame)

    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (21, 21), 0)

    if cameras[camera_name]['last_frame'] is None:
        cameras[camera_name]['last_frame'] = gray
        return

    frame_delta = cv2.absdiff(cameras[camera_name]['last_frame'], gray)
